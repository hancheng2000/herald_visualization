...
class VWindow:
    ...
    vmin: float
    vmax: float
    expected_sign: int = 0      # +1 charge (positive), -1 discharge (negative), 0 = unknown
    prominence: Optional[float] = None
    width: Optional[float] = None      # points, not volts
    distance: Optional[int] = None     # points, not volts
    min_abs_amp: float = 0.0           # reject tiny peaks by |amplitude|
    keep: str = "all"                  # "all" or "max" (strongest by |amp|)
def find_peaks_in_windows(voltage: np.ndarray, dqdv: np.ndarray, windows: List[VWindow]) -> pd.DataFrame:
    """
    Identify local maxima in dQ/dV within user-defined voltage windows and (optionally) filter them by
    sign, prominence, amplitude, and selection policy.
    The function scans each window on the provided (voltage, dQ/dV) trace, uses
    `scipy.signal.find_peaks` to locate candidate peaks, maps them back to global indices,
    estimates left/right bases when available, and computes a small trapezoidal-area metric
    under each peak between its bases. Results are returned in a tidy table (one row per peak).
    Args:
        voltage (array-like):
            Voltage array (same length and order as dqdv). Units typically volts.
        dqdv (array-like):
            Incremental capacity values (dQ/dV) corresponding to `voltage`.
        windows (list[VWindow]):
            List of window definitions. Each VWindow supports:
              - name (str): Label for the window.
              - vmin, vmax (float): Voltage bounds to search within.
              - expected_sign (int, optional): +1 for positive peaks (charge),
                    -1 for negative peaks (discharge), 0 to ignore sign. Default 0.
              - prominence (float, optional): Pass-through to `find_peaks` to suppress small peaks.
              - width (int, optional): Minimum width in **points** (not volts) for `find_peaks`.
              - distance (int, optional): Minimum distance in **points** between peaks.
              - min_abs_amp (float, optional): Reject peaks with |amplitude| below this threshold.
              - keep (str, optional): Selection policy per window:
                    "all" (default) — keep all detected peaks,
                    "max" — keep the strongest by |amplitude|.
                (If you extended the function, other modes like "first", "last",
                 "largest_area", "closest", or "topk" may be supported.)
    Returns:
        pandas.DataFrame:
            A table with one row per retained peak and columns:
              - 'window' (str): Window name.
              - 'v_peak' (float): Peak voltage (V).
              - 'amplitude' (float): dQ/dV at the peak (keeps physical sign).
              - 'idx' (int): Global index of the peak in the original arrays.
              - 'left_base_idx' (int): Global index of left base (if available).
              - 'right_base_idx' (int): Global index of right base (if available).
              - 'area' (float): Trapezoidal area of dQ/dV between bases (V·(dQ/dV) units).
    Notes:
        - Set `expected_sign=+1` for charge-only windows and `-1` for discharge-only windows
          so that minor bumps of the wrong sign are ignored.
        - Tune `prominence`, `distance`, and `min_abs_amp` per window to reduce false positives.
        - The 'area' metric is a simple local integral and is useful for relative comparisons
          (e.g., tracking peak strength vs. cycle), not for absolute thermodynamic quantities.
    Example:
        >>> windows = [
        ...     VWindow('lowV', 2.50, 3.10, expected_sign=-1, prominence=0.01, keep='max'),
        ...     VWindow('highV', 3.10, 3.60, expected_sign=+1, prominence=0.02),
        ... ]
        >>> peaks_df = find_peaks_in_windows(voltage, dqdv, windows)
        >>> peaks_df[['window','v_peak','amplitude']]
"""
    voltage = np.asarray(voltage); dqdv = np.asarray(dqdv)
    rows = []
    for w in windows:
        m = (voltage >= w.vmin) & (voltage <= w.vmax)
        if not np.any(m):
            continue
        v_win = voltage[m]; y_win = dqdv[m].copy()
        # For negative expected peaks, invert before peak search
        y_search = -y_win if w.expected_sign == -1 else y_win
        kwargs: Dict = {}
        if w.prominence is not None: kwargs['prominence'] = w.prominence
        if w.width is not None: kwargs['width'] = w.width
        if w.distance is not None: kwargs['distance'] = w.distance
        pk_idx, props = find_peaks(y_search, **kwargs)
        gidx = np.where(m)[0]  # local→global index map
        # Collect candidates
        for i in pk_idx:
            gi = int(gidx[i])
            amp = float(dqdv[gi])  # true sign preserved
            # Enforce sign (if specified)
            if w.expected_sign == +1 and amp <= 0: 
                continue
            if w.expected_sign == -1 and amp >= 0:
                continue
            # Enforce minimum amplitude
            if abs(amp) < w.min_abs_amp:
                continue
            # Bases for an area estimate (if provided)
            lb = int(props.get('left_bases', [i])[0]) if 'left_bases' in props else max(0, i-1)
            rb = int(props.get('right_bases', [i])[0]) if 'right_bases' in props else min(len(v_win)-1, i+1)
            gi_lb = int(gidx[lb]); gi_rb = int(gidx[rb])
            area = float(np.trapz(dqdv[gi_lb:gi_rb+1], voltage[gi_lb:gi_rb+1]))
            rows.append({
                'window': w.name,
                'v_peak': float(voltage[gi]),
                'amplitude': amp,
                'idx': gi,
                'left_base_idx': gi_lb,
                'right_base_idx': gi_rb,
                'area': area
            })
    df = pd.DataFrame(rows)
    if df.empty:
        return df
    # If requested, keep only the strongest peak per window
    if any(w.keep == "max" for w in windows):
        # make a little helper mapping window -> keep mode
        keep_map = {w.name: w.keep for w in windows}
        out = []
        for wname, grp in df.groupby('window', as_index=False):
            if keep_map.get(wname, "all") == "max":
                out.append(grp.iloc[[np.argmax(np.abs(grp['amplitude'].values))]])
            else:
                out.append(grp)
